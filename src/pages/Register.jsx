import React, { useState } from "react";
import "./register.scss";
import { useNavigate } from "react-router-dom";
import axios from "axios";
import configAPI from "../utils/configAPI";
export const Register = () => {
  const navigate = useNavigate();
  const [isRegistred, setRegistred] = useState(false);
  const [errorRegistred, setErrorRegistred] = useState(false);
  const [formData, setFormData] = useState({
    login: "",
    email: "",
    password: "",
    tryPassword: "",
  });

  const formSumbit = async (e) => {
    e.preventDefault();

    try {
      const response = await axios.post(
        configAPI.serverAuthURL + "/register",
        formData
      );
      if (response.status == 200) {
        console.log("register successful", response.status);
        setRegistred(true);
        setErrorRegistred(false);
      }
    } catch (err) {
      console.log("123", err);

      setErrorRegistred(true);
    }
  };
  const formChange = (e) => {
    const { name, value } = e.target;
    setFormData({ ...formData, [name]: value });
  };
  return (
    <div className="wrapper-reg">
      <div className="container-reg">
        <div className="section-left">
          <h1 className="title">Регистрация</h1>

          <form onSubmit={formSumbit} className="auth-form">
            <div className="form-box">
              <div className="form-left">
                <input
                  className="input"
                  name="login"
                  value={formData.login}
                  onChange={formChange}
                  type="text"
                  placeholder="Логин"
                />
                <input
                  className="input"
                  name="password"
                  value={formData.password}
                  onChange={formChange}
                  type="password"
                  placeholder="Пароль"
                />
                <input
                  className="input"
                  name="tryPassword"
                  value={formData.tryPassword}
                  onChange={formChange}
                  type="password"
                  placeholder="Повторите пароль"
                />
              </div>
              <div className="form-right">
                <input
                  className="input"
                  name="number"
                  type="number"
                  placeholder="Номер"
                />
                <input
                  className="input"
                  name="email"
                  value={formData.email}
                  onChange={formChange}
                  type="email"
                  placeholder="Электронная почта"
                />
                <input
                  className="input"
                  name="name"
                  type="text"
                  placeholder="ФИО"
                />
              </div>
            </div>

            <div className="btn-group">
              {!isRegistred && (
                <button className="sumbit" type="sumbit">
                  Отправить
                  <svg
                    className="arrow"
                    xmlns="http://www.w3.org/2000/svg"
                    width="26"
                    height="4"
                    viewBox="0 0 26 4"
                    fill="none"
                  >
                    <path
                      d="M25.1768 2.17678C25.2744 2.07915 25.2744 1.92085 25.1768 1.82322L23.5858 0.232233C23.4882 0.134602 23.3299 0.134602 23.2322 0.232233C23.1346 0.329864 23.1346 0.488155 23.2322 0.585786L24.6464 2L23.2322 3.41421C23.1346 3.51184 23.1346 3.67014 23.2322 3.76777C23.3299 3.8654 23.4882 3.8654 23.5858 3.76777L25.1768 2.17678ZM0 2.25H25V1.75H0V2.25Z"
                      fill="#FFFDF5"
                    />
                  </svg>
                </button>
              )}
              <button
                onClick={() => navigate("/login")}
                className="sign-in"
                type="sumbit"
              >
                Войти в кабинет
              </button>
            </div>
          </form>
          {isRegistred && (
            <p className="resp-ok">Регистрация прошла успешно</p>
          )}
          {errorRegistred && (
            <p className="resp-err">Ошибка валидации, попробуйте ввести корректные данные снова</p>
          )}
        </div>
        <div className="section-right">
          <div onClick={() => navigate("/")} className="button-close">
            <img src="/src/assets/Image/Ellipse7.svg" alt="" />
            <img
              className="svg"
              src="/src/assets/Image/close-line.svg"
              alt=""
            />
          </div>

          <img
            className="svg-bottom"
            src="/src/assets/Image/EllipseCircle.png"
            alt=""
          />
        </div>
      </div>
    </div>
  );
};
